...
/* ----------------------------------------------------------- 
    Program 6: 1024 with Graphic Interface and Undo 
    
    Class: CS 141, Spring 2019.  
    System: Codio
    Author: Ethan Luong
...
//---------------------------------------------------------------------
// Node struct holdes score, board, move number and pointer to next node
struct Node{
    Node* pNext;
    int score;
    int board[144];
...
//---------------------------------------------------------------------
// prepends new node to linkedl ist
void updateNode(Node *&pHead, int score, int moveNumber, int board[], int squaresPerSide)
{
    Node *pTemp = new Node;
    //stores current values to new node
    pTemp->score = score;
    pTemp->moveNumber = moveNumber;
    copyBoard(pTemp->board, board, squaresPerSide);
    pTemp->pNext = pHead;
    
    pHead = pTemp;
}
//---------------------------------------------------------------------
//Returns numbere of nodes in list
int getListSize(Node *pHead)
{
    int size = 0;
    //Goes through linkes list incrementing size until reaches NULL
    while(pHead != NULL)
    {
        size += 1;
        pHead = pHead->pNext;
...
//---------------------------------------------------------------------
//displays list size with correct formatting
void displayListSize(Node *pHead)
{
    //Gets size of list
...
{	
    ...
    for(int row = 0; row < squaresPerSide; ++row)
        {
            int shiftVal = row * squaresPerSide;
            for(int col = 0; col < squaresPerSide; ++col)
            {
                // Set each array pointer have the address of a new Square, created with a Square constructor
                squaresArray[ col + shiftVal] = Square(80, 80 * col + col*10, row*80 + row*10, sf::Color::White, true, "");
            }
        }
    ...
    updateNode(pHead, score, moveNumber, board, squaresPerSide);
	...
	while (window.isOpen())
	{
        ...
        for(int row = 0; row < squaresPerSide; ++row)
        {
            ...
            for(int col = 0; col < squaresPerSide; ++col)
            {
                ...
                if( board[col + shiftVal] == 0) { 
                    strcpy( name, "");   // "print" a blank text string
                }
                else {
                   sprintf( name, "%d", board[col + shiftVal]);   // "print" the square creation order number into a string to be stored in the square
                }            
                // Set each array pointer have the address of a new Square, created with a Square constructor
                squaresArray[ col + shiftVal].setText(name);
                // Draw the square
                window.draw( squaresArray[col + shiftVal].getTheSquare());
                // Draw the text associated with the Square, in the window with the indicated color and text size
                squaresArray[col + shiftVal].displayText( &window, font, sf::Color(0, 0, 0), 30);
            }
        }
        sprintf (aString, "Move %d", moveNumber);     //create string of correct movenumber
        ...
        std::cin >> userInput;
        //Initialize pointer to equal pHead to be able to deallocate on undo
        Node *free = pHead;                 
        switch (userInput) {
            case 'x':
            ...
            case 'u':
                    ...
                    if(getListSize(pHead) == 1)
                    {
                        cout << "        *** You cannot undo past the beginning of the game. Please retry. ***" << endl;
                        continue;
                        break;
                    }
                    //sets all variables to be the previous boardstate
                    cout << "        * Undoing move *" << endl;
                    pHead = pHead->pNext;
                    score = pHead->score;
                    moveNumber = pHead->moveNumber;
                    copyBoard(board, pHead->board, squaresPerSide);
                    //Frees original pHead pointer
                    delete free; 
                    //resets the window
                    window.clear(); 
                    continue;
